...
![img.png](pictures/architecture.png)

## File Structure

In the backend we decided to go for an uncomplicated monorepo. Therefor every service got its own directory. We created
...
In the frontend we kept the old structure, for more information please refer to the [old documentation](https://gitlab.mi.hdm-stuttgart.de/pm086/learnmoto/-/wikis/02-Architecture)
...
-config                                 --- Configuration (Styles, configfiles...)
-features                               --- every feature gets its own dir
    |-feature_name
        |-compontent                    --- every widget that is used in this feature
...
As we are planing to replace the app next semester anyway, the primary goal during this phase was to ensure a functional
